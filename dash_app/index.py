...
def load_network(network_params, bio_network):
    ...
    cyto_network = dict()
    cyto_network['elements'], cyto_network['nodes'], cyto_network['edges'], network_main_comp = \
        app_vis.make_cyto_elements(main_comp_network)
    print(type(cyto_network['elements']))
    ...
    return [json.dumps(cyto_network), network_df.to_json(), enrichment_results.to_json()]
...
def display_page(pathname, bio_network, enrichment_results, network_params):
    ...
    if pathname == '/':
        ...
        if bio_network is not None:
            # Load JSON data
            json_cyto_network, json_df, json_enrichment_results = \
            ...
            return vis_layout, json_df, json_cyto_network
        else:
